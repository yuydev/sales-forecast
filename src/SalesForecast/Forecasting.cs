...
public static class HoltWintersForecaster
{
        ...
        bool refitOnValidation = true)
    {
        ...
        var selected = seasonal is not null && improvement >= SeasonalImprovementThreshold
            ? seasonal
            : nonSeasonal;

        // 长历史序列用训练集+验证集重新拟合，再预测独立测试集。
        var forecastLength = refitOnValidation ? trainLength + horizon : trainLength;
        var forecast = Forecast(
            values.Take(forecastLength).ToArray(),
            selected.ModelType,
            selected.Alpha,
            selected.Beta,
            selected.Gamma,
    ...
    private static ForecastResult Evaluate(IReadOnlyList<double> values, ForecastModelType type, double alpha, double beta, double gamma, int seasonLength, int trainLength, int horizon)
    {
        ...
        var validationEnds = BuildValidationEnds(trainLength, horizon, minimumTrain);

        foreach (var end in validationEnds)
        {
            try
            {
                var predicted = Forecast(values.Take(end).ToArray(), type, alpha, beta, gamma, seasonLength, horizon);
                var actual = values.Skip(end).Take(horizon).ToArray();
                metrics.Add(CalculateMetrics(actual, predicted));
            }
        ...
        var actualAverage = values.Take(trainLength).DefaultIfEmpty().Average();
        var score = Score(average, actualAverage);
    ...
    {
        // 使用多个滚动窗口，最后一个窗口覆盖训练集末端，避免只对单一月份过拟合。
    ...
    private static double[] Holt(IReadOnlyList<double> values, double alpha, double beta, int horizon)
    {
        var level = values[0];
        ...
        return Enumerable.Range(1, horizon).Select(x => Math.Max(0, level + x * trend)).ToArray();
    }
    ...
    {
        if (values.Count < seasonLength * 2 || values.Any(x => x < 0 || double.IsNaN(x) || double.IsInfinity(x)))
        ...
        for (var i = 0; i < seasonLength; i++)
        {
            season[i] = multiplicative
                ? Safe(values[i] / Math.Max(firstAverage, Epsilon)) * 0.5 + Safe(values[i + seasonLength] / Math.Max(secondAverage, Epsilon)) * 0.5
                : ((values[i] - firstAverage) + (values[i + seasonLength] - secondAverage)) / 2;
        }
        ...
        for (var i = seasonLength; i < values.Count; i++)
        {
            ...
            level = multiplicative
                ? alpha * (values[i] / Math.Max(season[seasonIndex], Epsilon)) + (1 - alpha) * (level + trend)
                : alpha * (values[i] - season[seasonIndex]) + (1 - alpha) * (level + trend);
            trend = beta * (level - previousLevel) + (1 - beta) * trend;
            var component = multiplicative ? values[i] / Math.Max(level, Epsilon) : values[i] - level;
            season[seasonIndex] = Math.Max(multiplicative ? Epsilon : double.MinValue, gamma * component + (1 - gamma) * season[seasonIndex]);
        }

        return Enumerable.Range(1, horizon).Select(x =>
        {
            var seasonIndex = (values.Count + x - 1) % seasonLength;
            var baseline = level + x * trend;
            return Math.Max(0, multiplicative ? baseline * season[seasonIndex] : baseline + season[seasonIndex]);
        }).ToArray();
    }
